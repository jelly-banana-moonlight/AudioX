...
def generate_cond(
        ...
        video_path='/aifs4su/mmdata/rawdata/codeclm/trailer_3w/final_version/benchmark/benchmark/video/aWzlQ2N6qqg.mp4',
        seconds_start=0,
        seconds_total=30,
        cfg_scale=6.0,
        steps=250,
    ...
    length_set = 10
    # video_tensor = video_read_local('/aifs4su/data/tianzeyue/project/stable-audio-tools-unify-3/data/benchmark/_3kEH2ITVdI.mp4', seek_time=0, duration=length_set, target_fps=2)                    
    # video_tensor = video_read_local('/aifs4su/mmdata/rawdata/codeclm/trailer_3w/final_version/benchmark/benchmark/video/aWzlQ2N6qqg.mp4', seek_time=0, duration=length_set, target_fps=2)        
    print(f'video_path: {video_path}')
    video_tensor = video_read_local(video_path, seek_time=0, duration=length_set, target_fps=10)                    
    # video_tensor = video_read_local('/aifs4su/data/tianzeyue/project/VidMuse/data/video/M1UFYXQvvZ0_noaudio.mp4', seek_time=0, duration=length_set, target_fps=2)
    # video_tensor = video_read_local('/aifs4su/data/tianzeyue/project/VidMuse/data/video/fYyImx_KXm4_noaudio.mp4', seek_time=0, duration=length_set, target_fps=2)

    Video_tensors = [video_tensor.unsqueeze(0)]

    # Return fake stereo audio
    conditioning = [{"prompt": Video_tensors, "seconds_start": seconds_start, "seconds_total": seconds_total}] * batch_size
...
def create_sampling_ui(model_config, inpainting=False):
    with gr.Row():
        with gr.Column(scale=6):
            ...
            video_path = gr.Textbox(show_label=False, placeholder="Video path")
            # print(f'prompt:{prompt}, video_path:{video_path}')
        generate_button = gr.Button("Generate", variant='primary', scale=1)
